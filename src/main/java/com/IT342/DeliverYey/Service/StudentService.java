...
public class StudentService {
    ...
    public StudentEntity updateStudent(StudentEntity newStudentDetails){
        StudentEntity student = new StudentEntity();
        try {
            //1. search the ID  number of the student that will be updated
            student = studentRepository.findById(newStudentDetails.getSid()).get();
            ...
            student.setOrder(newStudentDetails.getOrder());
            student.setContactInfo(newStudentDetails.getContactInfo());
        }catch (NoSuchElementException ex){
            throw new NoSuchElementException("Student" + newStudentDetails.getSid() + "does not exist!");
        }
